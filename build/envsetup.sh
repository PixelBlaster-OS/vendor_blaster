...
cat <<EOF
Additional PixelBlaster functions:
...
- githubremote:    Add git remote for PixelBlaster Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [[ "$target" =~ -(user|userdebug|eng)$ ]]; then
            ...
            # This is probably just the PixelBlaster model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch blaster_$target-$variant
        fi
    fi
    return $?
}
...
{
    if [ "$OUT" ] ; then
        ZIPPATH=`ls -tr "$OUT"/PixelBlaster-*.zip | tail -1`
        if [ ! -f $ZIPPATH ] ; then
            echo "Nothing to eat"
            return 1
        fi
        ...
        echo "Found device"
        if (adb shell getprop ro.blaster.device | grep -q "$BLASTER_BUILD"); then
            echo "Rebooting to sideload for install"
            adb reboot sideload-auto-reboot
            adb wait-for-sideload
            adb sideload $ZIPPATH
        else
            echo "The connected device does not appear to be $BLASTER_BUILD, run away!"
        fi
        return $?
    else
        echo "Nothing to eat"
...
function githubremote()
{
    ...
    git remote add github https://github.com/PixelBlaster-OS/$PROJECT
...
function installboot()
{
    ...
    adb root
    adb wait-for-device-recovery
    if (adb shell getprop ro.blaster.device | grep -q "$BLASTER_BUILD");
    then
...
function dopush()
{
    ...
    if (adb shell getprop ro.blaster.device | grep -q "$BLASTER_BUILD") || [ "$FORCE_PUSH" = "true" ];
    then
...
function fixup_common_out_dir() {
    ...
    if [ ! -z $BLASTER_FIXUP_COMMON_OUT ]; then
