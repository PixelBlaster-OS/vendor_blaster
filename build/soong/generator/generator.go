...
func init() {
	android.RegisterModuleType("blaster_generator", GeneratorFactory)
}
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = blasterExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = blasterExpandVariables(ctx, dep_file)
	...
	cmd := blasterExpandVariables(ctx, String(g.properties.Cmd))
